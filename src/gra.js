        ...
        this.buduj = function()
        {
            ...
            for (i = 0; i < rozmiar; ++i)
            {
                ...
                for (j = 0; j < rozmiar; ++j)
                {
                    ...
                    plansza[i].push([opisPlanszy[miejsce], false]);
        ...
        this.uaktualnij = function(sciezka, kasuj)
        {
            ...
            for (k = 0; k < sciezka.length; k++)
            {
                ...
                if (kasuj && opisPlanszy[aktualny.substr(2,3)] == 0)
                    ...
                    plansza[i][j][0] = $('#' + sciezka[sciezka.length - 1]).attr('class').substr(4,5);
            }
        }
        
        function dfs(numer)
        {
            var cel, stopien, typ, akt = new Array(), stos = new Array();
            akt = zamienNaWspolrzedne(pozycje[numer][0]);
            cel = zamienNaWspolrzedne(pozycje[numer][1]);
            typ = opisPlanszy[pozycje[numer][0]];
            stos.push(akt);
            while (stos.length > 0)
            {
                akt = stos.pop();
                if (akt[0] == cel[0] && akt[1] == cel[1])
                    return true;
                stopien = 0;
                if (akt[0] > 0 &&
                    plansza[akt[0] - 1][akt[1]][0] == typ &&
                    plansza[akt[0] - 1][akt[1]][1] == false)
                {
                    plansza[akt[0] - 1][akt[1]][1] = true;
                    stos.push(new Array((akt[0] - 1), akt[1]));
                    ++stopien;
                }
                if (akt[0] + 1 < rozmiar &&
                    plansza[akt[0] + 1][akt[1]][0] == typ &&
                ...
                if (akt[1] > 0 &&
                    plansza[akt[0]][akt[1] - 1][0] == typ &&
                    plansza[akt[0]][akt[1] - 1][1] == false)
                {
                    plansza[akt[0]][akt[1] - 1][1] = true;
                    stos.push(new Array(akt[0], (akt[1] - 1)));
                    ++stopien;
                }
                if (akt[1] + 1 < rozmiar &&
                    plansza[akt[0]][akt[1] + 1][0] == typ &&
